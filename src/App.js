...
function App() {
  ...
  const formSubmit = async (e) => {
    ...
    try {
      ...
      await addYourAnimal({ animal: { name, description, amount, image }, amount: uploadFee })
      getAnimals();
    } catch (error) {
      console.log(error);
    }
          ...
          {animals.map(animal => <div className="col">
            <div className="card mb-4 rounded-3 shadow-sm">
              ...
              <div className="card-body">
                <h1 className="card-title pricing-card-title">${animal.amount / 10 ** 24}<small className="text-muted fw-light">cUSD</small></h1>
                <img width={200} src={animal.image} alt="" />
                <p className="list-unstyled mt-3 mb-4">
                  {animal.description}
                </p>
                {!animal.sold ? <button type="button"
